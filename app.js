const app = {
    ...
    renderCalculatorInputs(drug) {
        ...
        if (engine.inputs_required.length === 0) {
            html = `<div style="color: #4EC9B0; text-align: center; font-style: italic;">Posología universal estándar (No requiere parámetros biométricos)</div>`;
            inputsContainer.innerHTML = html;
            document.getElementById('calc-results').innerHTML = `<div style="text-align: center; color: #777;">Calculando...</div>`;
            setTimeout(() => app.calculateDose(), 50);
        } else {
            inputsContainer.innerHTML = html;
    ...
    calculateDose() {
        ...
        let cargaHtml = "";
        let maintHtml = "";
        const viaAdmin = drug.posologia.via_administracion || 'Definida por protocolo';
        ...
        if (engine.calculation_type === 'BANDAS_EDAD_PESO') {
            ...
            if (matchedRule.loading_output) {
                hasLoading = true;
                cargaHtml = `
                    <div class="calc-output" style="font-size: 1.05rem; margin-bottom: 0.5rem;">➔ <strong>Dosis a administrar:</strong> ${matchedRule.loading_output.dose_description}</div>
                    <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Vía de administración:</strong> ${viaAdmin}</div>
                    <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Periodicidad:</strong> ${matchedRule.loading_output.frequency_description}</div>
                    <div class="calc-output" style="margin-top: 1rem; color: #DCDCAA;">📦 <strong>Cajas a prescribir (MIPRES):</strong> ${matchedRule.loading_output.mipres_monthly_boxes}</div>
                `;
            }

            maintHtml = `
                <div style="color: #CE9178; margin-bottom: 1rem;">⚖️ Rango de peso/edad verificado.</div>
                <div class="calc-output" style="font-size: 1.05rem; margin-bottom: 0.5rem;">➔ <strong>Dosis a administrar:</strong> ${matchedRule.output.posology_am} ${matchedRule.output.posology_pm ? `+ ${matchedRule.output.posology_pm}` : ''}</div>
                <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Vía de administración:</strong> ${viaAdmin}</div>
                <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Periodicidad:</strong> ${matchedRule.output.frequency_description || drug.posologia.frecuencia_administracion}</div>
                <div class="calc-output" style="margin-top: 1rem; color: #DCDCAA;">📦 <strong>Cajas a prescribir (MIPRES):</strong> ${matchedRule.output.mipres_monthly_boxes}</div>
                ${matchedRule.output.food_requirement ? `<div class="calc-warning" style="margin-top:0.5rem;">📝 ${matchedRule.output.food_requirement}</div>` : ''}
            `;
        }

        else if (engine.calculation_type === 'LINEAL_MG_KG') {
            if (engine.fase_carga) {
                hasLoading = true;
                const dosisCargaTotal = peso * engine.fase_carga.dose_mg_kg;
                const resCarga = this.calculateVials(dosisCargaTotal, engine.fase_carga.vial_mg || engine.vial_mg);
                cargaHtml = `
                    <div style="color: #9CDCFE; margin-bottom: 0.5rem;">⚖️ Cálculo Lineal: ${engine.fase_carga.dose_mg_kg} mg/kg x ${peso} kg</div>
                    <div class="calc-output" style="font-size: 1.1rem; margin-bottom: 0.5rem;">➔ <strong>Dosis Total Calculada:</strong> ${resCarga.dosis_mg} mg</div>
                    <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Vía de administración:</strong> ${viaAdmin}</div>
                    <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Periodicidad:</strong> ${engine.fase_carga.frecuencia}</div>
                    <div class="calc-output" style="margin-top: 1rem; color: #DCDCAA;">📦 <strong>Viales a prescribir (MIPRES):</strong> ${resCarga.viales_a_solicitar} vial(es) de ${engine.fase_carga.vial_mg || engine.vial_mg}mg</div>
                    ${resCarga.alerta_desperdicio ? `<div class="calc-waste-alert">⚠️ Sobrante a desechar: ${resCarga.desperdicio_mg} mg</div>` : ''}
                `;
            }
            ...
            maintHtml = `
                <div style="color: #9CDCFE; margin-bottom: 0.5rem;">⚖️ Cálculo Lineal: ${engine.dose_mg_kg} mg/kg x ${peso} kg</div>
                <div class="calc-output" style="font-size: 1.1rem; margin-bottom: 0.5rem;">➔ <strong>Dosis Total Calculada:</strong> ${res.dosis_mg} mg</div>
                <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Vía de administración:</strong> ${viaAdmin}</div>
                <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Periodicidad:</strong> ${drug.posologia.frecuencia_administracion}</div>
                <div class="calc-output" style="margin-top: 1rem; color: #DCDCAA;">📦 <strong>Viales a prescribir (MIPRES):</strong> ${res.viales_a_solicitar} vial(es) de ${engine.vial_mg}mg</div>
            ...
            const res = this.calculateVials(dosisTotal, engine.vial_mg);
            maintHtml = `
                <div style="color: #9CDCFE; margin-bottom: 0.5rem;">⚖️ Sup. Corporal (Mosteller): ${bsa.toFixed(2)} m²</div>
                <div class="calc-output" style="font-size: 1.1rem; margin-bottom: 0.5rem;">➔ <strong>Dosis Total Calculada:</strong> ${res.dosis_mg} mg</div>
                <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Vía de administración:</strong> ${viaAdmin}</div>
                <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Periodicidad:</strong> ${drug.posologia.frecuencia_administracion}</div>
                <div class="calc-output" style="margin-top: 1rem; color: #DCDCAA;">📦 <strong>Viales a prescribir (MIPRES):</strong> ${res.viales_a_solicitar} vial(es) de ${engine.vial_mg}mg</div>
            ...
            if (engine.fase_carga) {
                hasLoading = true;
                const resCarga = this.calculateVials(engine.fase_carga.fixed_mg, engine.fase_carga.vial_mg || engine.vial_mg);
                cargaHtml = `
                    <div style="color: #9CDCFE; margin-bottom: 0.5rem;">⚖️ Umbrales superados. Dosis de Inducción.</div>
                    <div class="calc-output" style="font-size: 1.1rem; margin-bottom: 0.5rem;">➔ <strong>Dosis Total Calculada:</strong> ${engine.fase_carga.fixed_mg} mg</div>
                    <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Vía de administración:</strong> ${viaAdmin}</div>
                    <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Periodicidad:</strong> ${engine.fase_carga.frecuencia}</div>
                    <div class="calc-output" style="margin-top: 1rem; color: #DCDCAA;">📦 <strong>Viales/Cajas a prescribir (MIPRES):</strong> ${resCarga.viales_a_solicitar} de ${engine.fase_carga.vial_mg || engine.vial_mg}mg</div>
                `;
            }

            const res = this.calculateVials(engine.fixed_mg, engine.vial_mg);
            maintHtml = `
                <div style="color: #9CDCFE; margin-bottom: 0.5rem;">⚖️ Umbrales de inclusión superados (Dosis Fija)</div>
                <div class="calc-output" style="font-size: 1.1rem; margin-bottom: 0.5rem;">➔ <strong>Dosis Total Calculada:</strong> ${engine.fixed_mg} mg</div>
                <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Vía de administración:</strong> ${viaAdmin}</div>
                <div class="calc-output" style="margin-bottom: 0.5rem;">➔ <strong>Periodicidad:</strong> ${drug.posologia.frecuencia_administracion}</div>
                <div class="calc-output" style="margin-top: 1rem; color: #DCDCAA;">📦 <strong>Viales/Cajas a prescribir (MIPRES):</strong> ${res.viales_a_solicitar} de ${engine.vial_mg}mg</div>
            `;
        }

        if (hasLoading) {
            htmlOut = `
                <div style="background: rgba(86, 156, 214, 0.1); border-left: 3px solid #569CD6; padding: 0.75rem; margin-bottom: 1rem; border-radius: 0 4px 4px 0;">
                    <div style="color:#569CD6; font-weight:bold; margin-bottom: 0.5rem;">🟩 FASE DE CARGA (Inducción)</div>
                    ${cargaHtml}
                </div>
                <div style="background: rgba(78, 201, 176, 0.1); border-left: 3px solid #4EC9B0; padding: 0.75rem; border-radius: 0 4px 4px 0;">
                    <div style="color:#4EC9B0; font-weight:bold; margin-bottom: 0.5rem;">🟦 FASE DE MANTENIMIENTO</div>
                    ${maintHtml}
                </div>
            `;
        } else {
