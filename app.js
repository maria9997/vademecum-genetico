    ...
    calculateDose() {
        ...
        if (engine.calculation_type === 'BANDAS_EDAD_PESO') {
            ...
            if (matchedRule.loading_output) {
                hasLoading = true;
                cargaHtml = `
                    <div class="calc-output">• Dosis Carga: ${matchedRule.loading_output.dose_description}</div>
                    <div class="calc-output">• Cajas MIPRES: ${matchedRule.loading_output.mipres_monthly_boxes} caja(s)</div>
                    <div class="calc-warning">📝 ${matchedRule.loading_output.frequency_description}</div>
                `;
            }
            ...
            maintHtml = `
                ...
                ${matchedRule.output.posology_pm ? `<div class="calc-output">• Cantidad PM: ${matchedRule.output.posology_pm}</div>` : ''}
                <div class="calc-output">• Cajas / mes: ${matchedRule.output.mipres_monthly_boxes} caja(s)</div>
                <div class="calc-warning">📝 ${matchedRule.output.food_requirement || matchedRule.output.frequency_description || ''}</div>
            `;
        }
            ...
            if (engine.fase_carga) {
                ...
                cargaHtml = `
                    <div style="color: #9CDCFE;">⚖️ Cálculo Lineal: ${engine.fase_carga.dose_mg_kg} mg/kg x ${peso} kg</div>
                    <div class="calc-output" style="font-weight:bold;">Total a infundir: ${resCarga.dosis_mg} mg</div>
                    <div class="calc-output">Viales MIPRES: ${resCarga.viales_a_solicitar} vial(es)</div>
                    <div class="calc-warning">📝 ${engine.fase_carga.frecuencia}</div>
            ...
            if (engine.fase_carga) {
                ...
                cargaHtml = `
                    <div style="color: #9CDCFE;">⚖️ Dosis Fija de Inducción</div>
                    <div class="calc-output" style="font-weight:bold;">Total a infundir: ${engine.fase_carga.fixed_mg} mg</div>
                    <div class="calc-output">Viales MIPRES: ${resCarga.viales_a_solicitar} vial(es)</div>
                    <div class="calc-warning">📝 ${engine.fase_carga.frecuencia}</div>
                `;
            }
