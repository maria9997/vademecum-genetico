const drugsData = (function() {
    ...
    const invimaRecords = {
        "acido-carglumico": { numero: "INVIMA 2017M-0017688", cum: "201300-1" },
        "acido-colico": { numero: "INVIMA 2019M-0018742", cum: "201592-1" },
        "agalsidasa-alfa": { numero: "INVIMA 2008M-0008392", cum: "200054-1" },
        "agalsidasa-beta": { numero: "INVIMA 2015MBT-0004768-R1", cum: "19951126-1" },
        "alfa-1-antitripsina": { numero: "INVIMA 2014M-0014859", cum: "200845-1" },
        "alglucosidasa-alfa": { numero: "INVIMA 2008M-0008083", cum: "201024-2" },
        "asfotasa-alfa": { numero: "INVIMA 2018M-0018155", cum: "201467-1" },
        "avalglucosidasa-alfa": { numero: "INVIMA 2022M-0020815", cum: "201984-1" },
        "eliglustat": { numero: "INVIMA 2017M-0017997", cum: "201314-1" },
        "elosulfasa-alfa": { numero: "INVIMA 2015M-0015949", cum: "201114-1" },
        "galsulfasa": { numero: "INVIMA 2009M-0009667", cum: "200214-1" },
        "idursulfasa": { numero: "INVIMA 2008M-0007873", cum: "199723-1" },
        "imiglucerasa": { numero: "INVIMA 2006M-0005706", cum: "199142-1" },
        "laronidasa": { numero: "INVIMA 2007M-0007053", cum: "199522-1" },
        "migalastat": { numero: "INVIMA 2021M-0020085", cum: "201889-1" },
        "miglustat": { numero: "INVIMA 2010M-0010461", cum: "200421-1" },
        "nitisinona": { numero: "INVIMA 2016M-0016834", cum: "201277-1" },
        "pegvaliasa": { numero: "INVIMA 2021M-0020112", cum: "201999-1" },
        "sapropterina": { numero: "INVIMA 2012M-0013588", cum: "200788-1" },
        "taliglucerasa-alfa": { numero: "INVIMA 2015M-0016480", cum: "201183-1" },
        "velaglucerasa-alfa": { numero: "INVIMA 2013M-0014282", cum: "200921-1" },
        "inotersen": { numero: "INVIMA 2021M-0020288", cum: "201944-1" },
        "nusinersen": { numero: "INVIMA 2019M-0019253", cum: "201728-1" },
        "onasemnogen-abeparvovec": { numero: "INVIMA 2021M-0020100", cum: "201824-2" },
        "patisiran": { numero: "INVIMA 2021M-0020355", cum: "201893-1" },
        "risdiplam": { numero: "INVIMA 2021M-0020473", cum: "201988-1" },
        "trofinetida": { numero: "INVIMA 2024M-0021884", cum: "202399-1" },
        "voretigene-neparvovec": { numero: "INVIMA 2022M-0020993", cum: "202022-1" },
        "eculizumab": { numero: "INVIMA 2011M-0012569", cum: "200542-1" },
        "ravulizumab": { numero: "INVIMA 2022M-0020616", cum: "201977-1" },
        "ivacaftor": { numero: "INVIMA 2020M-0019973", cum: "201844-1" }
    };
    ...
    function buildBase(id, name, comm, atc, isVND, cats, icd10, inclusion, justif, presDesc, vialMg, freq, mech, expected, alarm, paraclinical) {
        let invimaNum = "Vigente (Validar número local)";
        let cumNum = "N/A";
        
        if (!isVND && invimaRecords[id]) {
            invimaNum = invimaRecords[id].numero;
            cumNum = invimaRecords[id].cum;
        }
        ...
        return {
            ...
            regulatorio: {
                registro_invima: { numero: isVND ? "N/A" : invimaNum, cum: isVND ? "N/A" : cumNum, vigente: !isVND },
            ...
            posologia: {
                ...
                via_administracion: (function(pd) {
                    pd = pd.toLowerCase();
                    if (pd.includes("tableta") || pd.includes("cápsula") || pd.includes("polvo oral") || pd.includes("solución 200mg/ml") || pd.includes("polvo 100g")) return "Vía Oral";
                    if (pd.includes("jeringa") || pd.includes("subcutáne")) return "Vía Subcutánea";
                    if (pd.includes("intratecal")) return "Vía Intratecal";
                    if (pd.includes("subretiniano")) return "Vía Subretiniana";
                    return "Vía Intravenosa (IV)";
                })(presDesc),
    ...
    function buildLinear(id, name, comm, atc, isVND, cats, icd10, inc, justif, presDesc, vialMg, doseMgKg, freq, mech, exp, al, par, loadingPhase = null) {
    ...
    function buildFixed(id, name, comm, atc, isVND, cats, icd10, inc, justif, presDesc, fixedMg, minAge, minWeight, freq, mech, exp, al, par, loadingPhase = null) {
    ...
    let eculizumab = buildBase("eculizumab", "Eculizumab", "Soliris", "L04AA25", false, ["Hematología / Enfermedades Mediadas por Complemento"], "D593 - Síndrome urémico hemolítico atípico", "SUHa o HPN severa.", "Anticuerpo monoclonal que se inhibe a C5 terminal.", "Vial 300mg", 300, "Cada 14 días", "Apaga el sistema inmunológico terminal que ataca los riñones.", "Cefalea, Nasofaringitis", "Fiebre y rigidez de cuello (MENINGITIS LETAL)", "Vacunación meningocócica mandatoria, LDH");
    eculizumab.posologia.motor_dosificacion = {
        calculation_type: "BANDAS_EDAD_PESO", inputs_required: ["peso_kg"], fallback: { trigger: "NINGUN_TIER_COINCIDE", mensaje: "Peso no soportado (< 5 kg).", accion_ui: "BLOQUEAR_CALCULO" },
        rules_matrix: [
            { tier: 1, conditions: { peso_kg: { operator: ">=", threshold: 40 } }, loading_output: { dose_description: "900 mg a la semana (Total: 4 dosis)", mipres_monthly_boxes: "12 cajas", frequency_description: "Días 1, 8, 15 y 22" }, output: { posology_am: "1200 mg", mipres_monthly_boxes: "8 cajas al mes", frequency_description: "Día 29 y luego cada 2 semanas" } },
            { tier: 2, conditions: { peso_kg: { min_inclusive: 30, max_exclusive: 40, operator: ">=", threshold: 30 } }, loading_output: { dose_description: "600 mg a la semana (Total: 2 dosis)", mipres_monthly_boxes: "4 cajas", frequency_description: "Días 1 y 8" }, output: { posology_am: "900 mg", mipres_monthly_boxes: "6 cajas al mes", frequency_description: "Día 15 y luego cada 2 semanas" } },
            { tier: 3, conditions: { peso_kg: { min_inclusive: 20, max_exclusive: 30, operator: ">=", threshold: 20 } }, loading_output: { dose_description: "600 mg a la semana (Total: 2 dosis)", mipres_monthly_boxes: "4 cajas", frequency_description: "Días 1 y 8" }, output: { posology_am: "600 mg", mipres_monthly_boxes: "4 cajas al mes", frequency_description: "Día 15 y luego cada 2 semanas" } },
            { tier: 4, conditions: { peso_kg: { min_inclusive: 10, max_exclusive: 20, operator: ">=", threshold: 10 } }, loading_output: { dose_description: "600 mg (Dosis única)", mipres_monthly_boxes: "2 cajas", frequency_description: "Día 1" }, output: { posology_am: "300 mg", mipres_monthly_boxes: "2 cajas al mes", frequency_description: "Día 8 y luego cada 2 semanas" } },
            { tier: 5, conditions: { peso_kg: { min_inclusive: 5, max_exclusive: 10, operator: ">=", threshold: 5 } }, loading_output: { dose_description: "300 mg (Dosis única)", mipres_monthly_boxes: "1 caja", frequency_description: "Día 1" }, output: { posology_am: "300 mg", mipres_monthly_boxes: "1 caja cada 3 sem", frequency_description: "Día 8 y luego cada 3 semanas" } }
        ]
    };
    list.push(eculizumab);

    let ravulizumab = buildBase("ravulizumab", "Ravulizumab", "Ultomiris", "L04AA43", false, ["Hematología / Enfermedades Mediadas por Complemento"], "D593 - Síndrome urémico hemolítico atípico", "SUHa o HPN.", "Inhibidor de C5 de acción extendida.", "Vial 300mg", 300, "Cada 8 semanas", "Apaga el ataque inmunológico con duración de meses.", "Infección de vías respiratorias", "Meningitis fulminante", "Vacuna meningocócica, LDH");
    ravulizumab.posologia.motor_dosificacion = {
        calculation_type: "BANDAS_EDAD_PESO", inputs_required: ["peso_kg"], fallback: { trigger: "NINGUN_TIER_COINCIDE", mensaje: "Peso no soportado (< 5 kg).", accion_ui: "BLOQUEAR_CALCULO" },
        rules_matrix: [
            { tier: 1, conditions: { peso_kg: { operator: ">=", threshold: 100 } }, loading_output: { dose_description: "3000 mg (Única dosis)", mipres_monthly_boxes: "10 cajas", frequency_description: "Día 1" }, output: { posology_am: "3600 mg", mipres_monthly_boxes: "12 cajas por dispensación", frequency_description: "Día 15 y luego cada 8 semanas" } },
            { tier: 2, conditions: { peso_kg: { min_inclusive: 60, max_exclusive: 100, operator: ">=", threshold: 60 } }, loading_output: { dose_description: "2700 mg (Única dosis)", mipres_monthly_boxes: "9 cajas", frequency_description: "Día 1" }, output: { posology_am: "3300 mg", mipres_monthly_boxes: "11 cajas por dispensación", frequency_description: "Día 15 y luego cada 8 semanas" } },
            { tier: 3, conditions: { peso_kg: { min_inclusive: 40, max_exclusive: 60, operator: ">=", threshold: 40 } }, loading_output: { dose_description: "2400 mg (Única dosis)", mipres_monthly_boxes: "8 cajas", frequency_description: "Día 1" }, output: { posology_am: "3000 mg", mipres_monthly_boxes: "10 cajas por dispensación", frequency_description: "Día 15 y luego cada 8 semanas" } },
            { tier: 4, conditions: { peso_kg: { min_inclusive: 30, max_exclusive: 40, operator: ">=", threshold: 30 } }, loading_output: { dose_description: "1200 mg (Única dosis)", mipres_monthly_boxes: "4 cajas", frequency_description: "Día 1" }, output: { posology_am: "2700 mg", mipres_monthly_boxes: "9 cajas por dispensación", frequency_description: "Día 15 y luego cada 8 semanas" } },
            { tier: 5, conditions: { peso_kg: { min_inclusive: 20, max_exclusive: 30, operator: ">=", threshold: 20 } }, loading_output: { dose_description: "900 mg (Única dosis)", mipres_monthly_boxes: "3 cajas", frequency_description: "Día 1" }, output: { posology_am: "2100 mg", mipres_monthly_boxes: "7 cajas por dispensación", frequency_description: "Día 15 y luego cada 8 semanas" } },
            { tier: 6, conditions: { peso_kg: { min_inclusive: 10, max_exclusive: 20, operator: ">=", threshold: 10 } }, loading_output: { dose_description: "600 mg (Única dosis)", mipres_monthly_boxes: "2 cajas", frequency_description: "Día 1" }, output: { posology_am: "600 mg", mipres_monthly_boxes: "2 cajas por mes", frequency_description: "Día 15 y luego cada 4 semanas" } },
            { tier: 7, conditions: { peso_kg: { min_inclusive: 5, max_exclusive: 10, operator: ">=", threshold: 5 } }, loading_output: { dose_description: "600 mg (Única dosis)", mipres_monthly_boxes: "2 cajas", frequency_description: "Día 1" }, output: { posology_am: "300 mg", mipres_monthly_boxes: "1 caja por mes", frequency_description: "Día 15 y luego cada 4 semanas" } }
        ]
    };
    list.push(ravulizumab);
    ...
    let lumasiran = buildBase("lumasiran", "Lumasiran", "Oxlumo", "A16AX18", true, ["Errores Innatos del Metabolismo (EIM)"], "E720 - Hiperoxaluria primaria tipo 1", "Diagnóstico de HP1.", "ARNi que bloquea glicolato oxidasa reduciendo oxalato.", "Vial 94.5mg", 94.5, "Trimestral", "Evita que el hígado forme cristales destructivos.", "Enrojecimiento", "Falla respiratoria", "Oxalato en orina");
    lumasiran.posologia.motor_dosificacion = {
        calculation_type: "BANDAS_EDAD_PESO", inputs_required: ["peso_kg"], fallback: { trigger: "NINGUN_TIER_COINCIDE", mensaje: "Requiere evaluación.", accion_ui: "BLOQUEAR_CALCULO" },
        rules_matrix: [
            { tier: 1, conditions: { peso_kg: { operator: ">=", threshold: 20 } }, loading_output: { dose_description: "3 mg/kg mensual x 3", mipres_monthly_boxes: "Manual", frequency_description: "Mensual x 3 dosis" }, output: { posology_am: "3 mg/kg", mipres_monthly_boxes: "Manual", frequency_description: "Trimestral a partir del mes 4" } },
            { tier: 2, conditions: { peso_kg: { operator: ">=", threshold: 10 } }, loading_output: { dose_description: "6 mg/kg mensual x 3", mipres_monthly_boxes: "Manual", frequency_description: "Mensual x 3 dosis" }, output: { posology_am: "6 mg/kg", mipres_monthly_boxes: "Manual", frequency_description: "Trimestral a partir del mes 4" } },
            { tier: 3, conditions: { peso_kg: { operator: "<", threshold: 10 } }, loading_output: { dose_description: "3 mg/kg mensual x 3", mipres_monthly_boxes: "Manual", frequency_description: "Mensual x 3 dosis" }, output: { posology_am: "3 mg/kg", mipres_monthly_boxes: "Manual", frequency_description: "Mensual continuo" } }
        ]
    };
    list.push(lumasiran);
